...
class Nasa_WC_Bulk_Discount {
    ...
    public function __construct() {
         ...
         */
        if (isset($_REQUEST['wc-ajax']) && $_REQUEST['wc-ajax'] == 'nasa_after_add_to_cart') {
        ...
        elseif(
            ...
            in_array ($_REQUEST['wc-ajax'], array('get_refreshed_fragments', 'nasa_reload_fragments'))
        ) {
    ...
    public function convert_price($price) {
        ...
        if (function_exists('wcml_is_multi_currency_on') && wcml_is_multi_currency_on()) {
            ...
            if ($woocommerce_wpml) {
                $multi_currencies = $woocommerce_wpml->get_multi_currency()->prices;
                $price = $multi_currencies->convert_price_amount($price);
            }
        }
    ...
    public function revert_price($price) {
        /**
         * Compatible - WPML - WooCommerce Multilingual
         * No use un-convert for WPML - WooCommerce Multilingual
         */
        // if (function_exists('wcml_is_multi_currency_on') && wcml_is_multi_currency_on()) {
            // global $woocommerce_wpml;
            ...
            // if ($woocommerce_wpml) {
                // $multi_currencies = $woocommerce_wpml->get_multi_currency()->prices;
                // $price = $multi_currencies->unconvert_price_amount($price);
            // }
        // }
