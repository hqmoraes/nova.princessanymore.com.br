...
function nasa_single_product_opts() {
    ...
    $layouts_role = array('classic', 'new', 'new-2', 'full', 'modern-1');
     ...
     * Product Layout
     */
    $single_layout = nasa_get_product_meta_value($product_id, 'nasa_layout');
    if ($single_layout) {
    ...
    elseif ($root_cat_id) {
         ...
         * Sidebar Layout
         */
        $_product_layout = get_term_meta($root_cat_id, 'single_product_layout', true);
     ...
     */
    $tab_style = nasa_get_product_meta_value($product_id, 'nasa_tab_style');
    if ($tab_style) {
     ...
     */
        ...
        if ($root_cat_id) {
            $tab_style = get_term_meta($root_cat_id, 'single_product_tabs_style', true);
            if ($tab_style) {
                $nasa_opt['tab_style_info'] = $tab_style;
            }
        }
...
function nasa_single_product_full_open_wrap() {
    echo '<div class="nasa-wrap-flex nasa-flex text-left rtl-text-right jst align-start">';
}
 ...
 * Loop layout buttons
 */
add_action('template_redirect', 'nasa_loop_layout_buttons');
