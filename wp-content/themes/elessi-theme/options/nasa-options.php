...
function elessi_enqueue_style() {
    ...
    if (NASA_ELEMENTOR_ACTIVE) {
        ...
        wp_enqueue_style('elessi-style-elementor', ELESSI_THEME_URI . '/style-elementor.css', array(), $themeVersion);
        
        if ($inMobile && isset($_REQUEST['elementor-preview']) && $_REQUEST['elementor-preview']) {
            wp_enqueue_style('elessi-style-large', ELESSI_THEME_URI . '/assets/css/style-large.css', array(), $themeVersion);
        }
    }
    ...
    if (isset($nasa_opt['transition_load']) && $nasa_opt['transition_load'] == 'crazy') {
         ...
         * CSS Crazy Load
         */
        wp_enqueue_style('elessi-style-crazy', ELESSI_THEME_URI . '/assets/css/style-crazy-load.css', array(), $themeVersion);
    }
     ...
     */
        ...
        wp_enqueue_style('elessi-style-mobile', ELESSI_THEME_URI . '/assets/css/style-mobile.css', array(), $themeVersion);
    }
     ...
     */
    if (NASA_RTL) {
        wp_enqueue_style('elessi-style-rtl', ELESSI_THEME_URI . '/style-rtl.css', array(), $themeVersion);
    }
    ...
    if (isset($_REQUEST['vc_editable']) && 'true' == $_REQUEST['vc_editable']) {
        wp_enqueue_style('elessi-wpbakery-frontend-editor', ELESSI_THEME_URI . '/wpbakery-frontend-editor.css', array(), $themeVersion);
    }
    ...
    if (defined('YITH_WCPB')) {
        wp_enqueue_style('elessi-style-yith_bundle', ELESSI_THEME_URI . '/assets/plgs3rd/style-yith_bundle.css', array(), $themeVersion);
    }
    ...
    if (defined('YITH_WAPO')) {
        wp_enqueue_style('elessi-style-yith-wc-add-ons', ELESSI_THEME_URI . '/assets/plgs3rd/style-yith-wc-add-ons.css', array(), $themeVersion);
    }
    ...
    if (elessi_check_blog_page()) {
        wp_enqueue_style('elessi-style-posts', ELESSI_THEME_URI . '/assets/css/style-posts.css', array(), $themeVersion);
    }
    ...
    if (NASA_WOO_ACTIVED) {
        ...
        if (
            ...
            wp_enqueue_style('elessi-loop-product', ELESSI_THEME_URI . '/assets/css/style-loop-product-' . $nasa_opt['loop_layout_buttons'] . '.css', array(), $themeVersion);
        }
        ...
        if (NASA_DOKAN_ACTIVED) {
            wp_enqueue_style('elessi-style-dokan-store', ELESSI_THEME_URI . '/assets/css/style-dokan-store.css', array(), $themeVersion);
        }
        ...
        if (is_shop() || is_product_taxonomy()) {
            ...
            wp_enqueue_style('elessi-style-archive-products', ELESSI_THEME_URI . '/assets/css/style-archive-products.css', array(), $themeVersion);
        }
         ...
         */
        if (is_product()) {
            wp_enqueue_style('elessi-style-signle-product', ELESSI_THEME_URI . '/assets/css/style-single-product.css', array(), $themeVersion);
        }
         ...
         */
            ...
            if (defined('ADSW_URL') && defined('ADSW_ASSETS_PATH')) {
                if (defined('ADSW_PATH') && is_file(ADSW_PATH . ADSW_ASSETS_PATH . 'css/front/adsw-style.min.css')) {
                    wp_enqueue_style('elessi-adsw-vaiation-style', ADSW_URL . ADSW_ASSETS_PATH . 'css/front/adsw-style.min.css', array(), $themeVersion);
                }
            }
        }
        ...
        if (is_checkout() || is_cart() || (NASA_CORE_USER_LOGGED && is_account_page())) {
            wp_enqueue_style('elessi-style-woo-pages', ELESSI_THEME_URI . '/assets/css/style-woo-pages.css', array(), $themeVersion);
        }
        ...
        if (isset($nasa_opt['css_canvas']) && $nasa_opt['css_canvas'] == 'sync') {
            wp_enqueue_style('elessi-style-off-canvas', ELESSI_THEME_URI . '/assets/css/style-off-canvas.css', array(), $themeVersion);
        }
    }
    ...
    if (
        ...
        wp_enqueue_style('elessi-style-font-weight', ELESSI_THEME_URI . '/assets/css/style-font-weight-' . $nasa_opt['max_font_weight'] . '.css', array(), $themeVersion);
    }
}
...
function elessi_add_fonts_style() {
     ...
     */
         ...
         */
        wp_enqueue_style('elessi-font-awesome-style', ELESSI_THEME_URI . '/assets/font-awesome-4.7.0/css/font-awesome.min.css');

        /**
         * Add Font Pe7s
         */
        wp_enqueue_style('elessi-font-pe7s-style', ELESSI_THEME_URI . '/assets/font-pe-icon-7-stroke/css/pe-icon-7-stroke.css');
    }
    ...
    if (isset($nasa_opt['include_font_awesome_new']) && $nasa_opt['include_font_awesome_new']) {
        wp_enqueue_style('elessi-font-awesome-5-free-style', ELESSI_THEME_URI . '/assets/font-awesome-5.0.13/css/fontawesome-all.min.css');
    }
}
